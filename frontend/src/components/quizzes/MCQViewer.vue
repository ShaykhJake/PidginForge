  ...
  <div class="pa-2 calligraphy">
    <v-card class="desertsand calligraphy--text" min-width="300">
        ...
        <div v-if="!graded">
          <div v-for="(question, index) in questionSet" :key="index">
            <!-- Hide all questions, show only the one with index === to current question index -->
              <div v-show="index === currentQuestion" :style="question.stem_style === 'rtl' ? 'direction: rtl; text-align: right' : 'direction: ltr; text-align: left' ">
                
                <h3>{{ `${index + 1}) `  }}{{ question.stem }}</h3>

                <div :style="question.alternatives_style === 'rtl' ? 'direction: rtl; text-align: right' : 'direction: ltr; text-align: left' ">
                  <v-radio-group v-if="question.answer_key.length <= 1"
          ...
          <div v-if="graded">
            <h2>Quiz Feedback</h2>
            <div>
              Total Score: {{ userScore }} out of {{ totalPoints }} ({{ Math.round(userScore / totalPoints * 100) }}%)
              <div 
                v-for="(gradedResponse, index) in gradedResponses" 
                :key="index" 
                >
                <hr>
                Question #{{ index + 1}}: <span :class="gradedResponse.correct ? 'green--text' : 'red--text'">{{ gradedResponse.correct ? 'Correct!' : 'Incorrect'}}</span>
                <div class="pl-5">
                  <strong>Your Response: {{ userResponses[index] }}</strong>
                  {{ questionSet[index].stem }}
                    <div v-if="questionSet[index].select_type==='multiple'">
                      <div v-for="(alternative, altindex) in questionSet[index].alternatives" :key="altindex">
                        <div v-if="userResponses[index].includes(alternative.key) && questionSet[index].answer_key.includes(alternative.key)">
                          <v-icon class="green--text">mdi-checkbox-marked-outline</v-icon>
                          {{ questionSet[index].alternatives[altindex].text }}
                          <v-icon class="green--text">mdi-check-decagram</v-icon>
                        </div>
                        <div v-else-if="userResponses[index].includes(alternative.key) && !questionSet[index].answer_key.includes(alternative.key)">
                          <v-icon class="red--text">mdi-checkbox-marked-outline</v-icon>
                          {{ questionSet[index].alternatives[altindex].text }}
                          <v-icon class="red--text">mdi-check-close</v-icon>
                        </div>
                        <div v-else-if="!userResponses[index].includes(alternative.key) && questionSet[index].answer_key.includes(alternative.key)">
                          <v-icon class="red--text">mdi-checkbox-blank-outline</v-icon>
                          {{ questionSet[index].alternatives[altindex].text }}
                    ...
                    <div v-else>
                      <div v-for="(alternative, altindex) in questionSet[index].alternatives" :key="altindex">
                        <div v-if="alternative.key === userResponses[index] && alternative.key === questionSet[index].answer_key[0]">
                          <v-icon class="green--text">mdi-radiobox-marked</v-icon>
                          {{ questionSet[index].alternatives[altindex].text }}
                          <v-icon class="green--text">mdi-check-decagram</v-icon>
                        </div>
                        <div v-else-if="alternative.key === userResponses[index] && alternative.key != questionSet[index].answer_key[0]">
                  ...
                  <strong>Answer Explanation:</strong> {{ questionSet[index].explanation }}
                </div>
              </div>
            </div>
          </div>
        </v-card-text>
      <v-card-actions v-if="editing" class="sandstone">
      </v-card-actions>
      ...
      <v-card-actions v-if="!graded && !editing" class="sandstone">
          ...
          <v-spacer></v-spacer>
          <span class="subtitle-1">Question #{{currentQuestion+1}} of {{questionSet.length}}</span>
          <v-spacer />
          <v-btn 
      ...
      <v-card-actions v-if="graded && !editing" class="sandstone">
          <v-spacer />
            <v-btn @click="resetQuiz" v-if="graded" class="elements desertsand--text">
              Reset Quiz
            </v-btn>
          <v-spacer />
      </v-card-actions>
        ...
        if(question.select_type === "multiple"){
          ...
          for(let i = 0; i < question.alternatives.length; i++){
            ...
            if(question.answer_key.includes(question.alternatives[i].key)){
              ...
              if(this.userResponses[index].includes(question.alternatives[i].key)){
            ...
            gradedResponse.gradedAlternatives.push({ correct: alternativeCorrect, style: alternativeStyle, userSelected: userSelected})
          }
          ...
          gradedResponse.points = alternativePoints / question.alternatives.length

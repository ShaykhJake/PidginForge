      ...
      <v-card-text>
        ...
        >
          <template v-slot:item="data">
            ...
            <template v-else>
              <v-list-item-content>
                <v-list-item-title v-html="data.item.word"></v-list-item-title>
                <v-list-item-subtitle
                  >Language(s): {{ data.item.languages }}</v-list-item-subtitle
                >
                <v-list-item-subtitle
                  >Lexeme: {{ data.item.lexeme.lemma }}</v-list-item-subtitle
                >
              </v-list-item-content>
            </template>
          </template>
        ...
        >
          <v-card-title class="sandstone calligraphy--text py-1">
            ...
            <v-btn
              small
              @click="hidingDetails = !hidingDetails"
          ...
          <v-card-text v-show="!hidingDetails">
            <div>
              Curated by <strong>{{ l1Details.curator.username }}</strong> on
              {{ l1Details.curationdate }}<br />
            </div>
            ...
            >
              {{ language }} </v-chip
            ><br />
            <strong>Definitions ({{ l1Details.definitions.length }}):</strong
            ><br />
            <v-list
              two-line
            ...
            <strong>Grammar Notes ({{ l1Details.grammars.length }}):</strong
            ><br />
            <v-list
              two-line
              class="pa-0 overflow-y-auto"
            ...
            <strong
              >Context Sentences ({{ l1Details.sentences.length }}):</strong
            ><br />
            <v-list
              two-line
              class="pa-0 overflow-y-auto"
        ...
        <v-card
          v-if="showingL1Details && !loadingL1Details && l1Details.word_pairs"
          class="desertsand"
        >
          <v-card-title class="sandstone py-1 calligraphy--text">
            ...
            ({{ l1Details.word_pairs.length }}):
            <v-spacer></v-spacer>
            <v-btn
              small
              @click="hidingPairs = !hidingPairs"
          ...
          >
            ...
            >
              <v-list-item
                ...
                :key="wordPair.id"
                @click="
                  wordPair.id === selectedPairID
                    ? (selectedPairID = null)
                    : (selectedPairID = wordPair.id)
                "
                class="px-0"
              >
                <v-list-item-content
                  ...
                  :class="
                    wordPair.id === selectedPairID
                      ? 'primary lighten-2 px-2'
                      : 'white px-2'
                  "
                >
                  <v-list-item-title
                    v-text="wordPair.inflected_form_2.word"
                  ></v-list-item-title>
                  <v-list-item-subtitle>
                    Language(s):
                    <v-chip
                ...
                >
                  <v-list-item-title
                    v-text="wordPair.inflected_form_1.word"
                  ></v-list-item-title>
                  <v-list-item-subtitle>
                    Language(s):
                    <v-chip
          ...
          <v-card-text v-else>
            <p>
              This word currently has no pairings in the database. Please add a
              new pairing.
            </p>
          </v-card-text>
          ...
          <v-card-text>
              ...
              <template v-slot:item="data">
                ...
                <template v-else>
                  <v-list-item-content>
                    ...
                    <v-list-item-subtitle
                      >Language(s):
                      {{ data.item.languages }}</v-list-item-subtitle
                    >
                    <v-list-item-subtitle
                      >Lexeme:
                      {{ data.item.lexeme.lemma }}</v-list-item-subtitle
                    >
                  </v-list-item-content>
                </template>
              </template>
            ...
            >
              <v-card-title class="sandstone calligraphy--text py-1">
                ...
                <v-btn
                  small
                  @click="hidingL2Details = !hidingL2Details"
                  class="garbage desertsand--text"
                >
              ...
              <v-card-text v-show="!hidingL2Details">
                <div>
                  Curated by
                  <strong>{{ l2Details.curator.username }}</strong> on
                  {{ l2Details.curationdate }}<br />
                </div>
                <strong>Associated Lexeme:</strong><br />
                <div class="ml-5 mr-">
                ...
                ><br />
                <strong
                  >Definitions ({{ l2Details.definitions.length }}):</strong
                ><br />
                <v-list
                  two-line
                  class="pa-0 overflow-y-auto"
                ...
                <strong>Grammar Notes ({{ l2Details.grammars.length }}):</strong
                ><br />
                <v-list
                  two-line
                  class="pa-0 overflow-y-auto"
                ...
                <strong
                  >Context Sentences ({{ l2Details.sentences.length }}):</strong
                ><br />
                <v-list
                  two-line
                  class="pa-0 overflow-y-auto"

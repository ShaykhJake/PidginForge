        ...
        <div v-else>
          ...
          <div class="overline">
            ...
            <span class="primary--text font-weight-black">{{
              sentence.curator.username
            }}</span>
            on {{ sentence.curationdate }}
          </div>
          <hr class="my-2" />
          <div class="mt-3">
            <h3>
              Audio Recordings ({{ audioArray.length }})
              <v-btn class="primary" small @click="addAudio">Add</v-btn>
            </h3>

            <div class="listBox sandstone my-1" v-if="audioArray.length > 0">
              <v-row
              ...
              >
                ...
                <v-col class="body-1">
                  <div class="roundBox">Sentence Audio ID #{{ item.id }}</div>
                  <div class="overline">
                    Curated by
                    <span class="primary--text font-weight-black">
                ...
                >
                  ...
                  <v-btn small class="primary" @click="deleteAudio(item.id)">
                    Delete
                  </v-btn>
                </v-overlay>
              </v-row>
          ...
          <div class="mb-2 mt-3">
            <h3>
              Translations ({{ translationArray.length }})
              <v-btn @click="attachTranslation" class="primary" small
                >Add</v-btn
              >
            </h3>
            <div
            ...
            >
              <div v-for="item in translationArray" :key="item.id" class="mb-1">
                <v-row dense>
                  <v-col class="body-1">
                    <div
                      :style="`direction: ${item.direction}`"
                      class="roundBox"
                    >
                      {{ item.text }}
                    </div>
          ...
          <div class="mt-2">
            <h3>
              Linked Words/Terms ({{ inflectedWordArray.length }})
              <v-btn class="primary" small>Add</v-btn>
            </h3>
              ...
              >
                ...
                >
                  <div style="text-align:center;">
                    ...
                    <span class="overline"
                      >Would you like to view the parent lexeme? This will route
                      you away from the current page.
                    </span>
                    <br />
                    <v-btn
                      small
                    ...
                    <v-btn
                      ...
                      @click="
                        $router.push({
                          name: 'Lexeme-Curator',
                          params: { lexemeslug: item.lexeme.slug }
                        })
                      "
                    >
                      View Lexeme
    ...
    >
      <v-card class="desertsand">
        ...
        <v-card-text class="px-1">
          <div
            :style="`direction:${sentence.direction}`"
            class="sentenceBox title mt-2"
          >
            {{ sentence.text }}
          </div>
          <audio controls autoplay :src="audio" class="mt-3"></audio>
        </v-card-text>
...
export default {
  ...
  components: {
    ...
    AttachTranslation: () =>
      import("@/components/vocab/sentences/AttachTranslation.vue")
  },
  data: () => ({
    loading: true,

   ...
   <v-dialog v-model="dialog" max-width="375" persistent>
      <v-card class="desertsand">
         ...
         <v-card-text class="px-2">
            <div class="sentenceBox title" :style="`direction:${sentence.direction}`">
               {{ sentence.text }}
            </div>
            
            <div v-if="audio && !newRecording" class="my-4" style="text-align:center;">
               Current Recording: 
               <audio
                  :src="audio.audioFile"
                  controls
               ></audio>
               <v-btn small class="primary" @click="newRecording=true; audio=null;">New Recording</v-btn>
            </div>
         ...
         <v-card-actions class="sandstone">
            ...
            <v-btn class="primary" :disabled="!valid || !audio" @click="submitAudio" :loading="saving">
               Submit
            </v-btn>
         </v-card-actions>
      </v-card>

  ...
  <v-dialog v-model="dialog" scrollable height=400 max-width=425>
     ...
     <v-card>
         ...
         <transition name="cardfade">
            <v-card min-height=200 v-if="showcard" @click="flip" pa-0>
               ...
               <v-container fill-height fluid>
                  <v-row align="center" justify="center" no-gutters style="height: 175px;">
            ...
            <div class="mt-1">
               <audio v-if="sideA && pairs[currentPair].lexeme_1_audio.audio_file" controls :src="pairs[currentPair].lexeme_1_audio.audio_file"></audio>
               <audio v-if="sideB && pairs[currentPair].lexeme_2_audio.audio_file" controls :src="pairs[currentPair].lexeme_2_audio.audio_file"></audio>
      ...
      <v-card-actions class="sandstone">
         <v-spacer />
         <v-btn class="desertsand" :disabled="currentPair==0" @click="currentPair--">Previous</v-btn>
         <v-spacer />
         {{ currentPair + 1 }} of {{ pairs.length }}
         <v-spacer />
         <v-btn class="desertsand" :disabled="currentPair==pairs.length-1" @click="currentPair++">Next</v-btn>
         <v-spacer />
      </v-card-actions>        
     </v-card>

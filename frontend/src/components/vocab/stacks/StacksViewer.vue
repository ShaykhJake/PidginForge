<template>
  <v-container>
    ...
    <div v-if="selectedStack" class="mt-3">
      <StackCurator :slug="selectedStack" :key="selectedStack" />
    </div>
  </v-container>
</template>
...
export default {
  ...
  data: () => ({
    ...
    rules: {
      ...
      maxName: value =>
        (value || "").length <= 200 || "Must be less than 120 characters",

      maxDescription: value =>
        (value || "").length <= 200 || "Must be less than 200 characters",

      requiredName: value =>
        (value || "").length > 3 || "Name must be at least 4 characters long.",
    ...
    headers: [
      ...
      { text: "# of Cards", value: "pair_count" },
      { text: "Mastery", value: "stats.mastery" },

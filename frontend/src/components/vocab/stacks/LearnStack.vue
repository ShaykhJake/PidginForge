<template>
  <v-container fluid class="calligraphy">
     <div v-if="!stack.id">
        Loading
     </div>
     <div v-if="stack.id">
        <v-card> 
           <v-card-title class="sandstone">
               Learn Stack: {{ stack.name }} ({{stack.lexeme_pairs.length}} Pairs)
           </v-card-title>
           <v-card-text class="desertsand">
               Stack Details:<br>
               Curator: {{ stack.curator.username }}<br>
               Created on: {{ stack.curationdate }}<br>
               Language Pair: {{ stack.learning_language }} -> {{ stack.native_language }}<br>
         ...
         <v-card-actions class="sandstone">
            ...
            <v-btn disabled class="mr-2 desertsand" @click="mixAndMatchDialog=true">
               Mix & Match
            </v-btn><br>
            <v-btn disabled class="mr-2 desertsand">
               Cloze
            </v-btn><br>
            <v-btn disabled class="mr-2 desertsand">
               Speech

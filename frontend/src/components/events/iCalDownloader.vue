<template>
  ...
  <v-card class="sandstone calligraphy--text">
    ...
    <v-card-text class="desertsand calligraphy--text pa-2">
      ...
      for {{ exportList.length > 1 ? `${exportList.length} events` : '1 event'}}.
    </v-card-text>
    ...
    <v-card-actions class="sandstone">
      ...
      <v-spacer></v-spacer>
      <v-btn :loading="generatingFile" @click="contentWriter" class="primary">Download<v-icon right>mdi-send</v-icon></v-btn>
    </v-card-actions>
  </v-card>
  </v-dialog>
</template>

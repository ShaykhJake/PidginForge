...
class CalendarEventViewSet(viewsets.ModelViewSet):
   ...
   def list(self, request):
      ...
      queryset = (CalendarEvent.objects.filter(public = True) |
         CalendarEvent.objects.filter(user_invite__in = userinvites))
   
      # The following is for when groups are implemented:
      # usergroups = request.user.group_member.all()
      # queryset = (CalendarEvent.objects.filter(public = True) |
      #    CalendarEvent.objects.filter(user_invite__in = userinvites) |
      #    CalendarEvent.objects.filter(group_invite__in = usergroups))
